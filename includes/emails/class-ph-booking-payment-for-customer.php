 ...
 *
 * @class   Ph_WC_Email_Booking_Payment
 * @extends WC_Email
 */
class Ph_WC_Email_Booking_Payment extends WC_Email {
	...
	public function __construct() {
		...
		$this->title          = __( 'PH Manual Bookings', 'bookings-and-appointments-for-woocommerce' );
		$this->description    = __( 'Bookings confirmation email with Payment Link for manually created bookings.', 'bookings-and-appointments-for-woocommerce' );

		$this->heading        = __( 'Thank you for booking with us', 'bookings-and-appointments-for-woocommerce' );
		
		$this->subject        = __( 'Your Booking Is Placed Successfully', 'bookings-and-appointments-for-woocommerce' );

		$this->customer_email = true;
		$this->template_html  = 'emails/ph-booking-payment-for-customer-html.php';

		$this->template_plain = 'emails/ph-booking-payment-for-customer-plain.php';
	 ...
	 * @return void
	 */
	public function trigger( $to_email, $status, $order_id, $order='' ) 
	{
		...
		if ( !$this->is_enabled() || !$to_email )
			return;

		// send the email!
		$this->send( $to_email, $this->get_subject(), $this->get_content(), $this->get_headers(), $this->get_attachments() );
...
return new Ph_WC_Email_Booking_Payment();
