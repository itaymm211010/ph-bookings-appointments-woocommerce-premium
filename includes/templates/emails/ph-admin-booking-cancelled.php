<?php
/**
 * PH Admin booking cancelled email HTML.
 * 
...
<!-- The Booking for the order #9099 has been cancelled -->
<p>
	<?php
		printf( __( 'The Booking for the order #%s has been cancelled.', 'bookings-and-appointments-for-woocommerce'), $order->get_order_number() );
	?>
</p>

<span style='color:<?php echo $email_base_color;?>;font-size:20px;'>
	<?php
		printf( '['.__( 'Order','bookings-and-appointments-for-woocommerce').' <a href="'.admin_url("admin.php?page=bookings&ph_booking_status=canceled").'" target="_blank" >#%s</a>] (%s)', $order->get_order_number(), ph_wp_date($wp_date_format) );
	?>
</span>
<br><br>
