...
/**
 * PH Customer booking requires confirmation email Plain.
 * 
...
echo sprintf(__('Hi', 'bookings-and-appointments-for-woocommerce') . ' ' . $customer_full_name);
echo sprintf(__("\n Thanks for placing your booking.", 'bookings-and-appointments-for-woocommerce'));
echo sprintf(__("\n We request you to go through the booking details attached in this email. \n", 'bookings-and-appointments-for-woocommerce'));
...
if (is_object($order)) 
{
	...
	if ($order_total > 0 && $wc_order_status != 'processing' && $wc_order_status != 'completed') {
		...
		echo sprintf(__( "\n Please click below link to proceed with the payment \n",'bookings-and-appointments-for-woocommerce')."<br><a href='".$order->get_checkout_payment_url()."' target='_blank' >".$order->get_checkout_payment_url().'</a>');
	}
}
